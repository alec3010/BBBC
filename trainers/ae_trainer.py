...
class AETrainer(Trainer):
    ...
    def __init__(self, env_name, configs) -> None:
        super(AETrainer,self).__init__(env_name=env_name, configs=configs)
        print("Creating AETrainer Object")
        self.train_x, self.train_y, self.val_x, self.val_y  = h.train_val_split(self.db_path_belief, self.split, self.idx_list)
        if self.prev_acs:
            self.model = AutoEncoder(self.obs_dim + self.acs_dim, self.acs_dim, self.acs_encoding_dim, self.obs_dim, self.belief_dim, self.hidden_dim, self.decoder_hidden, self.k, self.prev_acs).cuda()
        else:
            self.model = AutoEncoder(self.obs_dim, self.acs_dim, self.acs_encoding_dim, self.obs_dim, self.belief_dim, self.hidden_dim, self.decoder_hidden, self.k, self.prev_acs).cuda()
    ...
    def train(self):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for epoch in range(self.epochs):
                ...
                for (x, y) in zip(self.train_x, self.train_y):
                    ...
                    future_acs = torch.squeeze(h.t_tpk(y, k), 2)
                    past_acs = torch.squeeze(h.tmk_t(y, k), 2)
                    y_pad = h.zero_pad(y)
                    y_prev = y_pad[:-k-1]       
                    self.model.init_hidden()

                    pred, mu, mu_list = self.model(x[:-k], y_prev, future_acs=future_acs, past_acs=past_acs) # vae, pytorch , mu_s, log_sigma_s 
                    pred_labels = h.get_pred_labels(x, y, k)
                    losses, loss = self.reg_loss(pred, pred_labels)
        ...
        self.model.save(self.ae_state_dict)
    ...
    def eval(self, epoch):     
        ...
        for (x, y) in zip(self.val_x, self.val_y):
            ...
            valid_loss_rec += losses['reconstruction'].item()
            # valid_loss_obs_1fwd += losses['obs_one_fwd'].item()
            # valid_loss_obs_1bwd += losses['obs_one_bwd'].item()
            valid_loss_obs_kfwd += losses['obs_k_fwd'].item()
            valid_loss_obs_kbwd += losses['obs_k_bwd'].item()
            # valid_loss_acs_1fwd += losses['acs_1_fwd'].item()
            # valid_loss_acs_kfwd += losses['acs_k_fwd'].item()
