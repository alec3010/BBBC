...
class VAETrainer(Trainer):
    ...
    def __init__(self, env_name, configs) -> None:
        ...
        self.model = GRUVAE(self.obs_dim, self.belief_dim, self.hidden_dim, self.decoder_hidden).cuda()
        self.init_optimizer()
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for (x, y) in loader:
                ...
                pred, mu_s, sigma_s = self.model(x[k:-k]) # vae, pytorch
    ...
    def eval(self, epoch):
        ...
        n_iters = 0
        for (x, y) in loader:
            pred, mu_s, sigma_s, hn = self.model(x[k:-k]) # vae, pytorch
